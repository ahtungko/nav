  ...
  return {
    list(): Promise<WebsiteRow[]> {
      return d1All<WebsiteRow>(
        env.DB.prepare(`
          SELECT websites.id, websites.title, websites.url, websites.favicon_url, websites.category_id, websites.sort_order, websites.is_visible, websites.created_at, websites.updated_at
          FROM websites
    ...
    getById(id: string): Promise<WebsiteRow | null> {
      return d1First<WebsiteRow>(
        env.DB.prepare(`
          SELECT id, title, url, favicon_url, category_id, sort_order, is_visible, created_at, updated_at
          FROM websites
          WHERE id = ?1
        `).bind(id),
      );
